...
**카테고리명:** (예: **상의:**, **팬츠:**, **원피스/세트:** 등, 데이터에 실제로 존재하는 카테고리명 사용)

- 데이터에서 확인된 급상승 트렌드 헤드라인과 배경 분석. 개별 상품명이나 브랜드명을 나열하지 말고, 트렌드 패턴만 설명하세요.
- 구체적인 소재, 디자인, 가격대 등 트렌드 특징을 불렛 포인트로 나누어 작성하세요.

각 카테고리 사이에는 반드시 빈 줄을 추가하세요. **왜 이 스타일이 떴는지** 트렌드 배경을 설명하되, 개별 상품명을 나열하지 마세요. **반드시 불렛 포인트 형식을 사용하여 여러 줄로 상세히 작성하세요.**
