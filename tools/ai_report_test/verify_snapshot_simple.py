...
def check_query_history(company_name: str, year: int, month: int, hours: int = 1):
    """쿼리 히스토리 확인 안내"""
    print(f"\n📊 쿼리 비용 확인:")
    print(f"   BigQuery 콘솔: https://console.cloud.google.com/bigquery?project={PROJECT_ID}")
    print(f"   1. 왼쪽 메뉴 → '쿼리 히스토리' 클릭")
    print(f"   2. 최근 {hours}시간 필터 적용")
    print(f"   3. 검색: '{company_name}' 또는 '{year}-{month:02d}'")
    print(f"   4. 또는 check_snapshot_query_cost.sql 파일 실행")
