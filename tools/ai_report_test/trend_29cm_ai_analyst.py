...
def build_trend_analysis_prompt(snapshot_data: Dict) -> str:
    ...
    prompt = f"""당신은 여성 의류 쇼핑몰 MD를 위한 수석 데이터 분석가입니다.
제공된 29CM 랭킹 데이터를 분석하여, 다음 3가지 섹션으로 구성된 트렌드 리포트를 작성하세요.

**[중요: 작성 원칙]**
1. **완전한 문장 구사:** 개조식(~함, ~임)이나 단어 나열을 금지합니다. 반드시 **'~습니다.', '~입니다.'체**를 사용하여 주어와 서술어가 있는 완전한 문장으로 서술하세요.
   - (X) 소재: 플리스 유행. 보온성 중요.
   - (O) 이번 주는 **'플리스'** 소재가 유행하며, 보온성이 중요한 구매 요인으로 떠올랐습니다.
2. **데이터 근거 필수:** 순위 변동폭, 가격, 상품명을 구체적으로 언급하여 근거를 제시하세요.
   - (X) 비터셀즈가 인기다.
   - (O) **'비터셀즈'**의 **'플리스 하프 집업'**이 **44계단 상승**하며 인기를 끌고 있습니다.
3. **브랜드명 표기:** JSON 데이터에 있는 브랜드명(한글/영어)을 그대로 사용하세요. 절대 생략하거나 빈칸(** **)으로 두지 마세요.
   - JSON 데이터에 `"Brand": "어반드레스"`가 있으면 반드시 **'어반드레스'**로 표시하세요.
   - (X) `** **: '' "스트라이프 럭비 니트"` (브랜드명 생략 - 절대 안 됨!)
   - (O) **'어반드레스'**의 **'스트라이프 럭비 니트'**가 74계단 상승했습니다.

---

**[작성할 리포트 구조]**

## Section 1. Market Overview (시장 핵심 키워드)
전체 시장을 관통하는 3가지 흐름(소재, TPO, 가격)을 요약하여 줄글로 작성하세요.
* **Material (소재):** 유행하는 텍스처나 원단 (예: 플리스, 코듀로이 등)
* **Occasion (TPO):** 소비 목적 (예: 연초 모임, 홈웨어 등)
* **Price (가격):** 소비 패턴 (예: 가성비와 고가 아우터의 양극화 등)

## Section 2. Segment Deep Dive (세그먼트별 심층 분석)
제공된 데이터의 3가지 그룹(급상승, 신규진입, 순위하락)에 대해 상세히 분석하세요.
* **급상승:** 어떤 아이템이 트렌드를 주도하며 치고 올라오는지 분석하세요.
* **신규 진입:** 새롭게 등장한 브랜드나 고단가 아이템을 조명하세요.
* **순위 하락:** 시즌 아웃되거나 인기가 떨어진 스타일을 언급하세요.

## Section 3. Category Deep Dive (6대 핵심 카테고리)
다음 6개 카테고리 각각에 대해 트렌드를 1~2문장으로 요약하고, 대표적인 'Key Item'을 명시하세요.
...
⚠️ **매우 중요 - 브랜드명 처리 규칙**:
- JSON 데이터의 `Brand` 필드는 **한글이 포함될 수 있습니다** (예: "어반드레스", "비터셀즈", "수아레 우먼")
- JSON 데이터의 `Product` 필드도 **한글이 포함될 수 있습니다** (예: "스트라이프 럭비 니트")
- 브랜드명과 상품명을 인용할 때는 **반드시 JSON에 있는 원문을 그대로, 완전히** 사용하세요.
- **절대로** 한글 브랜드명을 영어로 번역하거나, 생략하거나, `** **`처럼 비워두지 마세요.

**올바른 작성 예시**:
- JSON: `{{"Brand":"어반드레스","Product":"스트라이프 럭비 니트","Rank_Change":74,"Price":74000}}`
- ✅ **'어반드레스'**의 **'스트라이프 럭비 니트'**가 74계단 상승하여 1위를 탈환했습니다.
- ✅ **'수아레 우먼'**의 **'라이트 시어쉘 패딩 점퍼'**가 급상승하며 인기를 끌고 있습니다.
"""
