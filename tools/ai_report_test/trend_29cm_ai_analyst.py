...
5. **Markdown Format:** 반드시 마크다운 형식을 사용하세요. **굵게**, *기울임*, 줄바꿈(빈 줄), 불렛 포인트(`*` 또는 `-`)를 적절히 활용하세요.
6. **Line Breaks:** 가독성을 위해 적절한 줄바꿈을 사용하세요. 카테고리, 세그먼트, 주요 항목 사이에는 반드시 빈 줄을 추가하세요.
...
각 카테고리별로 줄바꿈을 하여 분석하세요. 예를 들어:

* **상의:** (자사몰 상의 상품 성과 분석)
  * 구체적인 순위 변동과 인기 요인(소재, 디자인, 가격대 등)을 데이터 기반으로 작성

* **바지:** (자사몰 바지 상품 성과 분석)
  * 구체적인 순위 변동과 인기 요인을 데이터 기반으로 작성
...
데이터에서 확인된 소재 트렌드를 분석하세요. 데이터에 없는 소재는 언급하지 마세요. 구체적인 소재명과 트렌드 패턴을 언급하되, 예시 문구를 사용하지 마세요.

**Occasion (TPO):**

데이터에서 확인된 착용 시나리오나 소비 목적을 분석하세요. 실제 데이터에서 확인된 패턴만 언급하세요.

**Price (가격):**

데이터에서 확인된 가격대별 소비 패턴을 분석하세요. 구체적인 가격대와 소비 행태를 언급하세요.
...
⚠️ **매우 중요:** 
- **상품명, 브랜드명, 순위를 절대 나열하지 마세요.** 이 정보는 UI에서 썸네일 카드로 표시됩니다.
- 당신의 역할은 **트렌드 헤드라인과 분석 텍스트만 작성**하는 것입니다.
- 각 카테고리별로 줄바꿈을 하여 가독성을 높이세요.

**⚠️ 카테고리 헤드라인 형식 (필수 준수):**
- 각 카테고리는 반드시 `**카테고리명:**` 형식으로 시작해야 합니다. (예: `**상의:**`, `**바지:**`, `**스커트:**`, `**원피스:**`, `**니트웨어:**`, `**셋업:**`)
- 카테고리명은 정확하게 "상의", "바지", "스커트", "원피스", "니트웨어", "셋업" 중 하나를 사용하세요.
...
각 카테고리별로 반드시 다음 형식으로 작성하세요 (데이터가 있는 카테고리만):

**상의:**

데이터에서 확인된 급상승 트렌드 헤드라인과 배경 분석. 개별 상품명이나 브랜드명을 나열하지 말고, 트렌드 패턴만 설명하세요.

**바지:**

데이터에서 확인된 급상승 트렌드 헤드라인과 배경 분석.
...
각 카테고리 사이에는 반드시 빈 줄을 추가하세요. **왜 이 스타일이 떴는지** 트렌드 배경을 설명하되, 개별 상품명을 나열하지 마세요.
...
- 데이터에서 확인된 신규 진입 트렌드 헤드라인과 배경 분석. 개별 상품명이나 브랜드명을 나열하지 말고, 트렌드 패턴만 설명하세요.
- 구체적인 소재, 디자인, 가격대 등 트렌드 특징을 불렛 포인트로 나누어 작성하세요.

**바지:**

- 데이터에서 확인된 신규 진입 트렌드 헤드라인과 배경 분석.
- 구체적인 소재, 디자인, 가격대 등 트렌드 특징을 불렛 포인트로 나누어 작성하세요.
...
각 카테고리 사이에는 반드시 빈 줄을 추가하세요. 데이터에서 확인된 신규 진입 트렌드의 전반적 특징과 주요 흐름을 설명하되, 개별 상품명을 나열하지 마세요.
...
- 데이터에서 확인된 순위 하락 트렌드 헤드라인과 배경 분석. 개별 상품명이나 브랜드명을 나열하지 말고, 트렌드 패턴만 설명하세요.
- 구체적인 소재, 디자인, 가격대 등 트렌드 특징을 불렛 포인트로 나누어 작성하세요.

**바지:**

- 데이터에서 확인된 순위 하락 트렌드 헤드라인과 배경 분석.
- 구체적인 소재, 디자인, 가격대 등 트렌드 특징을 불렛 포인트로 나누어 작성하세요.
...
각 카테고리 사이에는 반드시 빈 줄을 추가하세요. 데이터에서 확인된 순위 하락 트렌드의 전반적 특징과 주요 흐름을 설명하되, 유행이 지나고 있는 소재나 디자인을 데이터 기반으로 제시하고 개별 상품명을 나열하지 마세요.
