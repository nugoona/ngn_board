...
**2. 테이블 데이터 (`table_data`) 작성 규칙:**
   ...
   - **값 길이:** 각 값은 **20자 이내**로 압축하여 작성하십시오.
...
{{
  "table_data": {{
    "주력 카테고리": {{ 
      "market": "[시장 베스트 상품의 핵심 카테고리 (예: 헤비 아우터, 무스탕)]", 
      "company": "[자사 베스트 상품의 핵심 카테고리 (예: 데일리 하의, 셋업)]" 
    }},
    "소비 패턴": {{ 
      "market": "[시장 소비 성향 (예: 계절감 있는 고단가 아이템 투자)]", 
      "company": "[자사 소비 성향 (예: 활용도 높은 가심비 아이템 구매)]" 
    }},
    "스타일 무드": {{ 
      "market": "[시장의 디자인 특징 (예: 오버핏, 소재감 강조)]", 
      "company": "[자사의 디자인 특징 (예: 베이직, 체형 보정)]" 
    }},
    "시장 기회": {{ 
      "market": "[시장의 틈새 (예: 고가 아우터 구매 후의 이너 니즈)]", 
      "company": "[자사의 공략 포인트 (예: 아우터 속 받쳐 입기 좋은 핏)]" 
    }}
  }},
  "card_summary": {{
    "market_analysis": "시장 트렌드 리포트:\\n[배경: 현재 29CM 시장을 주도하는 트렌드]\\n[트렌드: 주력 카테고리, 디자인 키워드, 소재, 컬러, 핏 스타일 등 구체적 트렌드 요소를 3~4문장으로 풍성하게 서술]\\n[기회: 시장 내에서 발견할 수 있는 기회 요인]",
...
""",
        9: f"""
[섹션 9: 데이터 기반 실무 MD 아이디어 (Action Cards)]
섹션 4(자사 효율)와 섹션 5(시장 트렌드) 데이터를 바탕으로, 매출에 즉각 도움을 줄 수 있는 **실행 가능한 전략 3가지**를 제안하십시오.

⚠️ **중요: 이 섹션 9만 분석하고 답변하세요.**

데이터:
- 리포트 대상 월: {report_month} (제안은 무조건 **{next_month}**을 기준으로 하십시오.)
- 자사몰 베스트 상품: {json.dumps(section_data.get('top_products', []), ensure_ascii=False, indent=2)} (Top 5~10까지의 데이터를 모두 검토하십시오.)

분석 요청 (Logic Selection):
아래 **4가지 로직** 중, 데이터 조건이 충족되는 **최적의 3가지**를 선택하여 작성하십시오.
(단, **1번 시즌 테마는 필수**로 포함해야 합니다.)

1.  **📅 (필수) 계절 전환 준비:**
    * {next_month}의 계절적 특징이나 이벤트(새해, 벚꽃, 바캉스, 개강 등)를 고려하여 해당 시기에 맞는 상품 기획을 제안하십시오.

2.  **💎 (조건부) 고효율 상품 발굴:**
    * 제공된 베스트 상품 리스트 전체(Top 10)에서 **'조회수는 상대적으로 낮은데 매출 상위권에 있는 상품'**을 찾으십시오.
    * 해당 상품의 전환율이 우수함을 강조하고, 노출 확대를 통한 매출 증대 방안을 제시하십시오.

3.  **🏷️ (조건부) 시장 키워드 활용:**
    * 섹션 5의 **시장 인기 키워드**가 우리 상품(Top 10 내)에도 적용 가능한지 확인하십시오.
    * 해당 키워드를 상품명이나 태그에 활용하여 검색 유입을 확대하는 방안을 제시하십시오.

4.  **🤝 (조건부) 상품 조합 전략:**
    * 베스트 상품 리스트(Top 10) 내에 **상의**와 **하의**가 동시에 존재할 때만 선택하십시오.
    * 두 상품을 묶어 세트 상품으로 구성하여 객단가 상승을 유도하는 방안을 제시하십시오.

⚠️ **출력 형식 (반드시 JSON)**:

**[규칙 1] 제목 생성:**
  * "데일리 코디 제안" 같은 뻔한 제목 금지. 반드시 **[약칭 상품명]** 또는 **[시즌 키워드]**를 포함하여 구체적으로 지을 것.
...
**[규칙 2] 상품명 약칭 사용:**
  ...
  * ✅ **핵심만 한글로 짧게(약칭) 부르십시오.**
    ...
    - 예: `[SET] Button Layered Skirt Pants_4Color` -> **`버튼 스커트 셋업`**
...
**[규칙 5] 마크다운 및 숫자:**
  * 강조가 필요한 핵심 단어(상품명, 키워드)는 `**단어**` 형태로 감쌀 것. (따옴표와 겹치지 않게 주의)
...
[
  {{
    "title": "📅 {next_month} 시즌 상품 기획",
    "content": "[{next_month}의 계절적 특징과 이벤트를 고려하여 해당 시기에 맞는 상품 기획 방안을 구체적으로 서술하십시오.]"
  }},
  {{
    "title": "💎 고효율 상품 노출 확대",
    "content": "[조회수 대비 매출이 우수한 상품명을 언급하고, 노출 확대를 통한 매출 증대 방안을 서술하십시오.]"
  }},
  {{
    "title": "🏷️ 시장 키워드 활용 전략",
    "content": "[시장 인기 키워드와 해당 키워드를 적용할 수 있는 자사 상품명을 언급하고, 검색 유입 확대 방안을 서술하십시오.]"
  }}
]
```
"""
