...
**2. 테이블 데이터 (`table_data`) 작성 규칙:**
   ...
   - ✅ **관계 설정:** 시장의 주류(Mainstream)에 대해 자사가 어떤 **보완적 역할(Complementary)**이나 **대안적 가치(Alternative)**를 제공하는지 대조하십시오.
...
{{
  ...
  "card_summary": {{
    "market_analysis": "시장 트렌드 리포트:\\n[불만 사항 제외. 60개 상품 데이터를 바탕으로 현재 시장을 주도하는 아이템, 소재, 컬러, 핏 트렌드를 객관적으로 서술]",
    "company_analysis": "포지셔닝 전략:\\n[시장이 '고가 아우터' 중심이라면 자사는 이를 받쳐주는 '탄탄한 이너'로 포지셔닝하는 등, 카테고리 차이를 인정한 현실적 역할 정의]"
  }}
}}
```
""",
...
섹션 4(자사 효율)와 섹션 5(시장 트렌드) 데이터를 바탕으로, 매출에 즉각 도움을 줄 수 있는 **기회 포착형 아이디어 3가지**를 제안하십시오.
...
1.  **📅 (필수) 계절의 시계는 한 달 앞서가기 (Month + 1):**
    * 리포트 월의 **다음 달** 날씨나 이벤트(새해, 벚꽃, 바캉스, 개강 등)를 예측하십시오.
    * 제안: "벌써 **[다음 달]**이네요. **[시즌 키워드]** 기획전으로 고객의 설렘을 자극해 보세요."

2.  **💎 (조건부) 숨은 보석 발굴 (고효율 상품):**
    * 제공된 베스트 상품 리스트 전체(Top 10)에서 **'조회수는 상대적으로 낮은데 매출 상위권에 있는 상품'**을 찾으십시오.
    * 인사이트: "이 상품, **구매 전환율(효율)**이 깡패입니다. 노출만 늘리면 매출이 뜁니다."
    * 제안: "메인 배너나 상단 노출로 **유입량**만 확보해 주면 효자 상품 등극이 확실해 보입니다."

3.  **🏷️ (조건부) 키워드 숟가락 얹기 (SEO):**
    * 섹션 5의 **시장 인기 키워드**가 우리 상품(Top 10 내)에도 적용 가능한지 확인하십시오.
    * 제안: "시장은 지금 **'[키워드]'** 열풍! 우리 **[상품명]** 태그나 설명에 이 단어를 슬쩍 넣어볼까요?"

4.  **🤝 (조건부) 검증된 꿀조합 (Cross-sell):**
    * 베스트 상품 리스트(Top 10) 내에 **상의**와 **하의**가 동시에 존재할 때만 선택하십시오.
    * 제안: "따로 팔아도 잘 나가는 **[상의]**와 **[하의]**, 묶으면 더 잘 나갑니다. **[5% 할인]** 등으로 객단가를 높여보세요."

⚠️ **출력 형식 (반드시 JSON - 마크다운 사용)**:
- **제목:** 매번 다르게 생성. 반드시 **[상품명]** 또는 **[시즌/시장 키워드]** 포함.
- **본문:** `**` 강조는 핵심 단어에만 사용. (따옴표 겹침 주의: `**단어**`)
- **톤앤매너:** 긍정적이고 의욕적인 MD 톤.

🛑 **주의: 아래 예시는 구조만 참고하십시오. 내용은 실제 데이터로 채우십시오.**

```json
[
  {{
    "title": "📅 미리 준비하는 **{next_month}** **[테마]** 얼리버드",
    "content": "어느덧 **{next_month}**이 다가오네요. **[계절/날씨]**에 딱 맞는 상품들을 모아 '**[감성 테마명]**'으로 기획전을 열면, 시즌 준비 고객을 선점할 수 있습니다."
  }},
  {{
    "title": "💎 효율 깡패 **[상품명]**, 노출만 늘리면 대박?",
    "content": "이 상품, 조회수는 **[조회수]**회로 적은데 매출은 **[매출액]**원이나 기록했어요. 본 사람들은 다 산다는 뜻이죠! **메인 배너**에 걸어서 유입만 늘려주면 효자 상품 등극이 확실해 보입니다."
  }},
  {{
    "title": "🏷️ 시장 핫키워드 **'[키워드]'**, 우리도 탑승!",
    "content": "지금 시장에서는 **'[키워드]'** 검색량이 많아요. 우리 **[상품명]**도 스타일이 비슷하니, 상품명이나 태그에 **'[키워드]'**를 추가해서 검색 유입을 노려보는 건 어떨까요?"
  }}
]
```
"""
