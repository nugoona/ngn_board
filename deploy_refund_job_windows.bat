@echo off
REM ngn-refund-job 배포 스크립트 (Windows)

REM 2-1. 변수 설정
for /f "tokens=2 delims==" %%I in ('wmic os get localdatetime /value') do set datetime=%%I
set TIMESTAMP=%datetime:~0,8%-%datetime:~8,6%
set PROJECT=winged-precept-443218-v8
set IMAGE=asia-northeast1-docker.pkg.dev/%PROJECT%/ngn-dashboard/ngn-refund-job:manual-%TIMESTAMP%

REM 2-2. Dockerfile을 루트로 복사
copy docker\Dockerfile-refund Dockerfile
