...
- `card_summary`는 **최소 3~4문장 이상의 구체적이고 풍부한 분석**을 포함하여 서술형으로 작성하십시오.
- **키 이름(Key)에 언더바(_)를 쓰지 말고 공백을 사용하십시오.**

**`card_summary` 작성 가이드 (매우 중요):**
- `market_analysis`: 시장 트렌드의 **배경, 주요 트렌드, 소비자 불만 사항, 기회 요인**을 포함하여 **3~4문장**으로 상세히 서술하십시오.
- `company_analysis`: 시장 불만을 해결하는 **자사몰의 구체적 강점, 가격 경쟁력, 타겟 고객층 정의, 차별화 전략**을 포함하여 **3~4문장**으로 상세히 서술하십시오.

```json
{{
  "table_data": {{
    "가격대": {{ 
      "market": "[시장 데이터에서 도출한 가격대 범위 (20자 이내)]", 
      "company": "[자사몰 데이터에서 도출한 가격대 범위 (20자 이내)]" 
    }},
    "주력 아이템": {{ 
      "market": "[시장 베스트 상품군 (20자 이내)]", 
      "company": "[자사 베스트 상품군 (20자 이내)]" 
    }},
    "타겟 고객층": {{ 
      "market": "[시장 주 타겟 특성 (20자 이내)]", 
      "company": "[자사 주 타겟 특성 (20자 이내)]" 
    }},
    "핵심 소재": {{ 
      "market": "[시장 주요 소재 (20자 이내)]", 
      "company": "[자사 주요 소재 (20자 이내)]" 
    }}
  }},
  "card_summary": {{
    "market_analysis": "**시장 트렌드 요약:**\\n\\n[첫 번째 문장: 시장의 전체적인 흐름과 주요 트렌드 설명]\\n\\n[두 번째 문장: 소비자의 구체적인 불만 사항(Pain Point)과 그 배경 설명]\\n\\n[세 번째 문장: 시장에서 발견되는 기회 요인과 틈새 시장 분석]\\n\\n[네 번째 문장: 시장의 강점과 약점을 종합적으로 평가]",
    "company_analysis": "**자사 포지셔닝:**\\n\\n[첫 번째 문장: 시장의 불만 사항을 해결하는 자사몰의 구체적 강점과 차별화 요소 설명]\\n\\n[두 번째 문장: 가격 경쟁력과 품질 대비 가치 제안 설명]\\n\\n[세 번째 문장: 타겟 고객층의 구체적 정의와 그들이 자사몰을 선택하는 이유]\\n\\n[네 번째 문장: 시장 대비 자사몰의 전략적 포지셔닝과 향후 방향성]"
  }}
}}
```
""",
...
섹션 4(자사몰 상품 효율)와 섹션 5(시장 불만) 분석을 결합하여, 실질적인 매출 증대 전략을 제안하십시오.

⚠️ **중요: 이 섹션 9만 분석하고 답변하세요.**

데이터:
- 자사몰 상품 데이터 (섹션 4): {json.dumps(section_data.get('top_products', []), ensure_ascii=False, indent=2)}
- 섹션 4 분석 결과: {json.dumps(section_data.get('section_4_analysis', ''), ensure_ascii=False, indent=2)}
- 섹션 5 분석 결과 (시장 트렌드): {json.dumps(section_data.get('section_5_analysis', ''), ensure_ascii=False, indent=2)}

🛑 **매우 중요: 상품명 사용 규칙**
- **반드시 위 '자사몰 상품 데이터'에 실제로 존재하는 상품명만 사용하십시오.**
- 섹션 5(시장 트렌드)의 경쟁사 상품명은 절대 사용하지 마십시오. 섹션 5는 시장 불만 사항을 파악하는 용도로만 사용하십시오.
- 존재하지 않는 상품명을 지어내지 마십시오.
- 상품명이 확실하지 않다면, 구체적인 상품명 대신 카테고리나 일반적인 설명으로 대체하십시오.

분석 요청:
단순 할인이 아닌, **데이터 로직에 기반한 구체적 타겟팅 전략**이어야 합니다.
**아래 3가지 로직(Logic)을 반드시 적용하여 작성하십시오.**

1. **💡 구매 전환 유도 (Bundling Strategy):**
   * 위 '자사몰 상품 데이터'를 분석하여 **'조회수는 높으나(관심) 구매 전환이 낮은 자사 상품'**을 찾으십시오.
   * 이를 **'매출 1위 자사 상품(검증된 상품)'**과 묶어(Set) 구매 장벽을 낮추고 객단가를 높이는 전략을 제안하십시오.

2. **🎯 틈새 공략 (Solution Marketing):**
   * 섹션 5의 **경쟁사 불만(털 빠짐, 배송 지연, 무거움 등)**을 해결해 줄 수 있는 **위 '자사몰 상품 데이터'에 있는 자사 상품(소재, 당일 발송 등)**을 찾아 소구하십시오.

3. **📦 재고 소진 (Inventory Clearance):**
   * 위 '자사몰 상품 데이터'에서 매출 하락세를 보이거나 시즌이 지나가는 자사 상품에 대해, **'명분(설 연휴, 시즌 마감 등)'**을 내세운 긴급 처분 전략을 세우십시오.

⚠️ **출력 형식 (반드시 JSON - 마크다운 사용)**:
1. **제목 생성 규칙 (동적 생성):**
   - 매번 똑같은 제목 금지. **제목에는 상품명을 포함하지 마십시오.** 전략의 핵심 키워드만 포함하십시오.
   - 형식: "이모지 1개 + 공백 + 전략 키워드만 포함한 제목"
   - (X) "💡 **[상품명 A]** & **[상품명 B]** 세트 제안" (상품명 포함 금지)
   - (O) "💡 겨울 코디 세트 제안" (전략 키워드만)
   - (O) "🎯 품질 강조 캠페인" (전략 키워드만)
   - (O) "📦 시즌 오프 클리어런스" (전략 키워드만)

2. **본문 작성 및 마크다운(`**`) 안전 규칙:**
   - **강조:** 본문에서 자사몰 상품명과 수치는 **굵게(`**`)** 표시하십시오.
   - 🛑 **깨짐 방지 (매우 중요):**
     - **따옴표 금지:** `**'상품명'**` (X) → **`**상품명**`** (O) (따옴표 없이 단어만 감싸십시오)
     - **특수문자 분리:** `**90%**` (X) → **`**90**%`** (O) (%는 볼드 바깥으로)
   - **가독성:** 줄바꿈(`\\n`)과 불렛 포인트(`•`)를 적극 사용하십시오.
   - **용어:** 영어 변수명(`is_declining`, `stock`) 사용 금지. **'매출 감소 추세', '재고'** 등 한국어로 자연스럽게 표현하십시오.

🛑 **주의: 아래 예시는 '구조'만 참고하는 템플릿입니다. 내용은 반드시 위 로직과 실제 자사몰 상품 데이터를 사용하여 새로 쓰십시오.**

```json
[
  {{
    "title": "💡 겨울 코디 세트 제안",
    "content": "섹션 4 분석 결과, **[자사몰 상품명 A]**은 높은 조회수에 비해 구매 전환이 아쉽습니다. 이를 매출 1위인 **[자사몰 상품명 B]**과 세트로 구성해 보세요.\\n\\n• **전략:** 두 상품을 묶어 '실패 없는 베스트 조합'으로 제안\\n• **효과:** 코디 고민 해결 및 객단가 동반 상승"
  }},
  {{
    "title": "🎯 품질 강조 캠페인",
    "content": "시장 내 **[경쟁사 주요 불만]**에 지친 고객을 타겟팅합니다. 자사 **[자사몰 상품명]**만이 가진 강점을 어필하십시오.\\n\\n• **소구점:** **[자사 상품의 소재/기능적 강점]** 집중 홍보\\n• **액션:** 경쟁사 대비 우위를 보여주는 콘텐츠 배포"
  }},
  {{
    "title": "📦 시즌 오프 클리어런스",
    "content": "매출 감소 추세를 보이는 **[자사몰 하락세 상품명]** 및 시즌 상품의 빠른 재고 소진이 필요합니다.\\n\\n• **전략:** **[명절/시즌 마감]** 임박을 명분으로 한 타임 세일 진행\\n• **체크:** 재고 소진 목표 설정 및 마진율 시뮬레이션"
  }}
]
```
"""
