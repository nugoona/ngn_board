...
섹션 4(상품 효율)와 섹션 5(시장 불만) 분석을 결합하여, 실질적인 매출 증대 전략을 제안하십시오.
...
1. **💡 구매 전환 유도 (Bundling Strategy):**
   * 섹션 4 데이터를 분석하여 **'조회수는 높으나(관심) 구매 전환이 낮은 상품'**을 찾으십시오.
   * 이를 **'매출 1위 상품(검증된 상품)'**과 묶어(Set) 구매 장벽을 낮추고 객단가를 높이는 전략을 제안하십시오.

2. **🎯 틈새 공략 (Solution Marketing):**
   * 섹션 5의 **경쟁사 불만(털 빠짐, 배송 지연, 무거움 등)**을 해결해 줄 수 있는 **자사 상품(소재, 당일 발송 등)**을 찾아 소구하십시오.

3. **📦 재고 소진 (Inventory Clearance):**
   * 매출 하락세(Declining)를 보이거나 시즌이 지나가는 상품에 대해, **'명분(설 연휴, 시즌 마감 등)'**을 내세운 긴급 처분 전략을 세우십시오.
...
1. **제목 생성 규칙 (동적 생성):**
   - 매번 똑같은 제목 금지. 반드시 **분석된 '실제 상품명'이나 '핵심 키워드'를 제목에 포함**하십시오.
   - 형식: "이모지 1개 + 공백 + [상품명/키워드] 포함 제목"
   - (X) "💡 번들링 전략 제안" (너무 뻔함)
   - (O) "💡 **[실제 상품명 A]** & **[실제 상품명 B]** 꿀조합 세트" (상품명 포함)

2. **본문 작성 및 마크다운(`**`) 안전 규칙:**
   - **강조:** 핵심 상품명과 수치는 **굵게(`**`)** 표시하십시오.
   - 🛑 **깨짐 방지 (매우 중요):**
...
🛑 **주의: 아래 예시는 '구조'만 참고하는 템플릿입니다. 내용은 반드시 위 로직과 실제 데이터를 사용하여 새로 쓰십시오.**

```json
[
  {{
    "title": "💡 **[매출 1위 상품명]** & **[관심 상품명]** 세트 제안",
    "content": "섹션 4 분석 결과, **[관심 상품명]**은 높은 조회수에 비해 구매 전환이 아쉽습니다. 이를 매출 1위인 **[효자 상품명]**과 세트로 구성해 보세요.\\n\\n• **전략:** 두 상품을 묶어 '실패 없는 베스트 조합'으로 제안\\n• **효과:** 코디 고민 해결 및 객단가 동반 상승"
  }},
  {{
    "title": "🎯 **[자사 강점 키워드]** 강조 캠페인",
    "content": "시장 내 **[경쟁사 주요 불만]**에 지친 고객을 타겟팅합니다. 자사 **[상품명]**만이 가진 강점을 어필하십시오.\\n\\n• **소구점:** **[자사 상품의 소재/기능적 강점]** 집중 홍보\\n• **액션:** 경쟁사 대비 우위를 보여주는 콘텐츠 배포"
  }},
  {{
    "title": "📦 **[시즌 이슈]** 맞이 **[재고 상품명]** 클리어런스",
    "content": "매출 감소 추세를 보이는 **[하락세 상품명]** 및 시즌 상품의 빠른 재고 소진이 필요합니다.\\n\\n• **전략:** **[명절/시즌 마감]** 임박을 명분으로 한 타임 세일 진행\\n• **체크:** 재고 소진 목표 설정 및 마진율 시뮬레이션"
  }}
]
```
"""
