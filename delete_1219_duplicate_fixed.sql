-- ✅ 12/19 중복 환불 삭제 (수정 버전)
-- 주의: 삭제 전에 check_1219_duplicate.sql로 확인하세요!

DELETE FROM `winged-precept-443218-v8.ngn_dataset.cafe24_refunds_table` r1
WHERE r1.mall_id = 'piscess1'
  AND DATE(DATETIME(TIMESTAMP(r1.refund_date), 'Asia/Seoul')) = '2025-12-19'
  AND EXISTS (
    SELECT 1
    FROM (
        SELECT 
        ...
        WHERE mall_id = 'piscess1'
          AND DATE(DATETIME(TIMESTAMP(refund_date), 'Asia/Seoul')) = '2025-12-19'
    ) r2
