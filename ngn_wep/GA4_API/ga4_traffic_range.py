"""
GA4 트래픽 데이터 날짜 범위 수집 스크립트
사용법: python ga4_traffic_range.py [start_date] [end_date]
예시: python ga4_traffic_range.py 2025-12-26 2026-01-02
"""
import sys
import pandas as pd
...
def collect_ga4_traffic(start_date, end_date):
    ...
    # ✅ 날짜 범위 생성 (start_date와 end_date 모두 포함)
    date_range = pd.date_range(start=start_date, end=end_date, inclusive='both').strftime("%Y-%m-%d").tolist()
    logging.info(f"📅 수집할 날짜 범위: {len(date_range)}일 ({date_range[0]} ~ {date_range[-1]})")
    all_rows_traffic = []
...
if __name__ == "__main__":
    # ✅ 명령줄 인자 확인
    if len(sys.argv) != 3:
        logging.error("❌ 사용법: python ga4_traffic_range.py [start_date] [end_date]")
        logging.error("   예시: python ga4_traffic_range.py 2025-12-26 2026-01-02")
        sys.exit(1)
    ...
    try:
        ...
        logging.error("❌ 날짜 형식이 올바르지 않습니다. YYYY-MM-DD 형식을 사용하세요.")
        sys.exit(1)
    
    logging.info(f"📅 날짜 범위 지정 모드: {start_date} ~ {end_date}")
    collect_ga4_traffic(start_date, end_date)
    update_ga4_traffic_ngn(start_date, end_date)
