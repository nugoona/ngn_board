...
try:
    ...
    # 3. 스크립트 위치 기준으로 프로젝트 루트 찾기 (ngn_board)
    if not env_loaded:
...
- **Section 1과 Section 2에서 절대 문단 형태로 작성하지 마세요.** 반드시 불렛 포인트(`*`)를 사용하여 작성해야 합니다. 문단 형태는 가독성이 떨어지고 잘림 현상이 발생합니다.
...
⚠️ **절대 문단 형태로 작성하지 마세요. 반드시 불렛 포인트(`*`)를 사용하여 작성하세요.**

각 카테고리는 다음과 같은 형식으로 작성하세요:

**카테고리명:**

* 이번 주 랭킹 데이터에서 자사 브랜드의 **상품명**이 카테고리에서 순위 변동을 보였습니다.
* 이 제품은 **디자인 요소1**, **디자인 요소2** 등이 조화를 이루며 소비자들의 높은 관심을 이끌어낸 것으로 분석됩니다.
* 가격대나 기타 인기 요인을 언급하세요.

**올바른 예시:**
**스커트:**

* 이번 주 랭킹 데이터에서 자사 브랜드 파이시스의 **버튼 레이어드 미디 스커트_블랙** 상품이 스커트 카테고리에서 25위 상승하며 강한 상승세를 보였습니다.
* 이 제품은 **버튼 레이어드 디테일**과 활용도 높은 **미디 기장**, 그리고 시크한 **블랙 컬러**가 조화를 이루며 소비자들의 높은 관심을 이끌어낸 것으로 분석됩니다.
* 44,100원의 합리적인 가격대 또한 접근성을 높여 주효했던 것으로 보입니다.

**잘못된 예시 (절대 사용하지 마세요):**
이번 주 랭킹 데이터에서 자사 브랜드 파이시스의 버튼 레이어드 미디 스커트_블랙 상품이 스커트 카테고리에서 25위 상승하며 강한 상승세를 보였습니다. 이 제품은 버튼 레이어드 디테일과 활용도 높은 미디 기장, 그리고 시크한 블랙 컬러가 조화를 이루며 소비자들의 높은 관심을 이끌어낸 것으로 분석됩니다. (문단 형태 - 금지!)

* 구체적인 소재명, 디자인 요소, 실루엣 등은 반드시 **굵게** 처리하세요.
* 각 불렛 포인트는 한 문장 또는 짧은 문단으로 작성하되, 내용이 완전히 전달되도록 하세요.
* 문장이 중간에 잘리지 않도록 완전한 문장으로 작성하세요.
...
⚠️⚠️⚠️ **절대 금지: 문단 형태로 작성하지 마세요. 반드시 불렛 포인트(`*`)를 사용하여 작성해야 합니다. 문단 형태로 작성하면 리포트가 거부됩니다.**

**필수 형식 (반드시 따라야 함):**
각 문장을 불렛 포인트(`*`)로 시작하고, 각 불렛 포인트는 새로운 줄에 작성하세요.

* 현재 시장에서는 겨울 시즌을 맞아 따뜻하면서도 고급스러운 **울**, **알파카**, **캐시미어** 등 천연 섬유 블렌드 소재의 니트와 아우터가 강세를 보이고 있습니다.
* 특히 **모헤어**나 **퍼지(Fuzzy) 니트**처럼 표면에 질감이 살아있는 소재들이 주목받고 있습니다.
* **코듀로이**와 **플리스**는 팬츠와 캐주얼 아우터에서 편안함과 보온성을 동시에 제공하며 인기를 견인하고 있습니다.
* 드레스나 스커트에서는 **벨벳**과 **새틴** 같은 광택감 있는 소재들이 연말연시 무드를 더하며 고급스러움을 표현하고 있습니다.

**절대 사용 금지 형식:**
현재 시장에서는 겨울 시즌을 맞아 따뜻하면서도 고급스러운 울, 알파카, 캐시미어 등 천연 섬유 블렌드 소재의 니트와 아우터가 강세를 보이고 있습니다. 특히 모헤어나 퍼지(Fuzzy) 니트처럼 표면에 질감이 살아있는 소재들이 주목받고 있으며... (이런 문단 형태는 절대 사용하지 마세요!)

* 데이터에서 확인된 소재 트렌드 패턴을 불렛 포인트로 나누어 작성하세요.
* 구체적인 소재명(예: **울**, **캐시미어**, **플리스**, **코듀로이**, **벨벳** 등)은 반드시 **굵게** 처리하세요.
* 데이터에 없는 소재는 언급하지 마세요.
* 각 불렛 포인트는 내용이 완전히 전달되도록 작성하되, 너무 길지 않게 적절한 길이로 유지하세요.
* 문장이 중간에 잘리지 않도록 완전한 문장으로 작성하세요.
* 마지막 불렛 포인트가 잘리지 않도록 주의하세요.

**⚠️ 마크다운 굵게 처리 주의사항:**
- `**굵게**` 처리 시, 앞뒤에 괄호 `()` 나 쉼표 `,` 같은 특수문자가 바로 붙으면 마크다운 효과가 사라질 수 있습니다.
- 올바른 예시: `**울**, **캐시미어**, **플리스**` 또는 `**소재명** (100% 울)`
- 잘못된 예시: `**소재명**(설명)` 또는 `**소재명**,` (특수문자가 바로 붙으면 파싱 실패)
- 특수문자 앞에는 공백을 두거나, 여러 키워드를 나열할 때는 각각 별도로 `**키워드1**, **키워드2**` 형식으로 작성하세요.
...
**올바른 예시:**
* 시장 전반적으로 편안하고 실용적인 **릴렉스 핏**과 **와이드 실루엣**이 강세를 유지하고 있습니다.
* 특히 **와이드 스웨트팬츠**, **릴렉스드 핏 데님**, **부츠컷 팬츠**, **오버핏 아우터**가 높은 인기를 얻고 있습니다.
* 동시에 **트위드**, **부클**, **페이크 퍼** 등 질감 있는 소재와 **빈티지 워싱** 데님, **노르딕 패턴** 니트 등 레트로 감성이 더해진 디자인들이 주목받고 있습니다.
* 여성스러운 무드로는 **러플**, **레이스 슬립 드레스**, **프릴 맥시 드레스**, **타이업 리본** 디테일이 강조된 원피스와 스커트가 급상승하며 로맨틱하고 우아한 스타일을 선호하는 경향이 뚜렷합니다.
* 로고 플레이는 **스몰 로고**, **캠퍼스 로고** 등 비교적 절제된 형태로 캐주얼 아이템에 활용되고 있습니다.

**잘못된 예시 (절대 사용하지 마세요):**
시장 전반적으로 편안하고 실용적인 릴렉스 핏과 와이드 실루엣이 강세를 유지하고 있습니다. 특히 와이드 스웨트팬츠, 릴렉스드 핏 데님, 부츠컷 팬츠, 오버핏 아우터가 높은 인기를 얻고 있습니다. 동시에 트위드, 부클, 페이크 퍼 등 질감 있는 소재와... (문단 형태 - 금지!)

* 데이터에서 확인된 시각적 분위기와 디자인적 특징을 불렛 포인트로 나누어 작성하세요.
* 구체적인 디자인 요소, 스타일 키워드, 실루엣, 디테일명(예: **로고 그래픽**, **빈티지 워싱**, **릴렉스 핏**, **스웨트 셋업**, **트위드 셋업**, **레이스 슬립 드레스**, **프릴 디테일**, **타이업 디자인** 등 데이터에서 확인된 실제 키워드)는 반드시 **굵게** 처리하세요.
* 데이터에서 나타난 실제 디자인 요소만 강조하세요.
* 각 불렛 포인트는 내용이 완전히 전달되도록 작성하되, 너무 길지 않게 적절한 길이로 유지하세요.
* 문장이 중간에 잘리지 않도록 완전한 문장으로 작성하세요.
* 마지막 불렛 포인트가 잘리지 않도록 주의하세요.
...
**⚠️ 마크다운 굵게 처리 주의사항:**
- `**굵게**` 처리 시, 앞뒤에 괄호 `()` 나 쉼표 `,` 같은 특수문자가 바로 붙으면 마크다운 효과가 사라질 수 있습니다.
- 올바른 예시: `**트위드 셋업**, **니트 세트**` 또는 `**오버핏** 실루엣` 또는 `**세트 구성** (3PCS, 4PCS)`
- 잘못된 예시: `**트위드 셋업**,` 또는 `**오버핏**,` (특수문자가 바로 붙으면 파싱 실패)
- 특수문자 앞에는 공백을 두거나, 여러 키워드를 나열할 때는 각각 별도로 `**키워드1**, **키워드2**` 형식으로 작성하세요.
...
* **트렌드 헤드라인:** 트렌드 배경 및 디테일 설명 문장. (헤드라인은 명사형으로 끝내고 굵게 처리, 설명 문장 내 핵심 패션 키워드도 반드시 굵게 처리)
* **트렌드 헤드라인:** 트렌드 배경 및 디테일 설명 문장. (헤드라인은 명사형으로 끝내고 굵게 처리, 설명 문장 내 핵심 패션 키워드도 반드시 굵게 처리)

**⚠️ 매우 중요:**
- 각 불렛 포인트의 **첫 글자부터 시작**하세요. 불렛 포인트(`*`) 다음에 바로 공백을 두고 텍스트를 작성하세요.
- 잘못된 예시: `*트렌드 키워드` (공백 없음)
- 올바른 예시: `* **트렌드 키워드**: 설명 문장`
- 문장이 중간에 잘리지 않도록 완전한 문장으로 작성하세요.

**⚠️ 마크다운 굵게 처리 주의사항:**
- `**굵게**` 처리 시, 앞뒤에 괄호 `()` 나 쉼표 `,` 같은 특수문자가 바로 붙으면 마크다운 효과가 사라질 수 있습니다.
- 올바른 예시: `* **트렌드 키워드** (설명)` 또는 `* **키워드1**, **키워드2** 등 설명`
- 잘못된 예시: `* **트렌드 키워드**(설명)` 또는 `* **트렌드 키워드**, 설명` (특수문자가 바로 붙으면 파싱 실패)
- 특수문자 앞에는 공백을 두거나, 여러 키워드를 나열할 때는 각각 별도로 `**키워드1**, **키워드2**` 형식으로 작성하세요.
- 헤드라인과 설명 사이에는 콜론(`:`) 뒤에 공백을 반드시 두세요: `* **헤드라인**: 설명`
