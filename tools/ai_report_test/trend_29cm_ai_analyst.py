...
데이터에서 확인된 소재 트렌드를 분석하세요. 데이터에 없는 소재는 언급하지 마세요. 구체적인 소재명과 트렌드 패턴을 언급하되, 예시 문구를 사용하지 마세요. (예: 겨울철 보온성을 강조한 울, 플리스, 벨벳 등)

**Occasion (TPO):**

데이터에서 확인된 착용 시나리오나 소비 목적을 분석하세요. 실제 데이터에서 확인된 패턴만 언급하세요. (예: 연말 모임룩과 편안한 데일리웨어의 공존)
