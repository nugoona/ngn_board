...
@app.route("/admake/crop")
def admake_crop_page():
    """ADMAKE 광고 만들기 Step 2 페이지 (크롭 가이드)"""
    if "user_id" not in session:
        return redirect(url_for("auth.login"))
    ...
    return render_template("admake_crop_page.html",
                           company_names=session.get("company_names", []),
                           account_id=account_id)

@app.route("/admake/adcreate")
def admake_adcreate_page():
    """ADMAKE 광고 만들기 Step 3 페이지 (AD 생성 & 미리보기)"""
    if "user_id" not in session:
        return redirect(url_for("auth.login"))
    ...
    return render_template("admake_adcreate_page.html",
                           company_names=session.get("company_names", []),
                           account_id=account_id)
...
@app.route("/admake/publish")
def admake_publish_page():
    """ADMAKE Step 5 페이지 (검토 및 생성)"""
    if "user_id" not in session:
        return redirect(url_for("auth.login"))
    ...
    return render_template("admake_publish_page.html",
                           company_names=session.get("company_names", []),
                           account_id=account_id)
