...
# 월간 AI 분석 Cloud Run Job 배포 스크립트
...
# 한국시간 오전 6시 5분 = UTC 21:05 (전날)
if gcloud scheduler jobs describe monthly-ai-analysis-scheduler --location="$REGION_RUN" --project="$PROJECT" &>/dev/null; then
  echo "스케줄러가 이미 존재합니다. 업데이트 중..."
  gcloud scheduler jobs update pubsub monthly-ai-analysis-scheduler \
    --location="$REGION_RUN" \
    --schedule="5 21 1 * *" \
    --topic="$TOPIC_NAME" \
  ...
  gcloud scheduler jobs create pubsub monthly-ai-analysis-scheduler \
    --location="$REGION_RUN" \
    --schedule="5 6 1 * *" \
    --topic="$TOPIC_NAME" \
    --message-body='{"trigger":"monthly"}' \
...
echo "  - Cloud Scheduler: monthly-ai-analysis-scheduler (매월 1일 오전 6시 5분 한국시간 실행)"
echo ""
echo "📝 수동 실행:"
