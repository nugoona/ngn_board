...
# event 시트 수집 Cloud Run Job 배포 스크립트
...
# Dockerfile 임시 복사
cp docker/Dockerfile-sheet-event-collector ./Dockerfile
...
SCHEDULER_NAME="sheet-event-collector-scheduler"
if gcloud scheduler jobs describe "$SCHEDULER_NAME" --location="$REGION_RUN" --project="$PROJECT" &>/dev/null; then
  ...
  echo "스케줄러 생성 중: 매월 1일 새벽 4시 20분"
...
echo "  - Cloud Scheduler: ${SCHEDULER_NAME} (매월 1일 새벽 4시 20분 실행)"
echo ""
echo "📝 수동 실행:"
