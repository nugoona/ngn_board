...
""",
        9: f"""
[섹션 9: {next_month} 전략 제안 (Action Cards)]
{next_month}에 실행할 수 있는 **전략 2가지**를 제안하십시오.

⚠️ **중요: 이 섹션 9만 분석하고 답변하세요.**

데이터:
- 리포트 대상 월: {report_month} (제안은 무조건 **{next_month}**을 기준으로 하십시오.)
- 자사몰 베스트 상품: {json.dumps(section_data.get('top_products', []), ensure_ascii=False, indent=2)} (판매 순위 1~30위 상품을 참고하십시오.)

분석 요청:
**2가지 카드**를 작성하십시오:
1. **이벤트 제안 카드 (1개):** {next_month}에 적합한 여성 패션의류 이벤트 제안
2. **고효율 상품 카드 (1개):** 판매율은 좋은데 조회수가 낮은 상품의 노출 확대 전략
...
**1. 이벤트 제안 카드:**
   ...
   - ✅ **판매 순위 1~30위 상품**에서 추천하십시오.
   - 🛑 **조회수는 높은데 판매량이 저조한 상품은 절대 추천하지 마십시오.**
   - ✅ 자사몰에 있는 카테고리 상품을 활용한 이벤트를 제안하십시오.

**2. 고효율 상품 카드:**
   - ✅ 제공된 베스트 상품 리스트 전체(Top 30)에서 **'조회수는 상대적으로 낮은데 매출 상위권에 있는 상품'**을 찾으십시오.
   - ✅ 해당 상품의 전환율이 우수함을 강조하고, 노출 확대를 통한 매출 증대 방안을 제시하십시오.
   - ✅ 상품명은 약칭으로 표기하십시오.

⚠️ **출력 형식 (반드시 JSON)**:

**[규칙 1] 제목 생성:**
  * 첫 번째 카드: 이벤트 제목은 **이벤트 유형과 시기**를 명확히 포함하십시오 (예: "🎁 신년 특가 이벤트", "❄️ 겨울 정리 세일").
  * 두 번째 카드: 고효율 상품 관련 제목 (예: "💎 고효율 상품 노출 확대", "⭐ 숨은 인기 상품 발굴").
...
**[규칙 3] 본문 포맷:**
  ...
  * 첫 번째 카드(이벤트): 이벤트 구성(할인율, 세트 구성, 무료배송, 쿠폰 등)을 명확히 제시하십시오.
  * 두 번째 카드(고효율 상품): 조회수 대비 매출이 우수한 상품명을 언급하고, 노출 확대를 통한 매출 증대 방안을 서술하십시오.
...
[
  ...
  {{
    "title": "💎 고효율 상품 노출 확대",
    "content": "• [조회수 대비 매출이 우수한 상품명을 약칭으로 언급]\\n• [해당 상품의 전환율 우수성 강조]\\n• [노출 확대를 통한 매출 증대 방안 서술]\\n• [기대 효과: 매출 증대, 효율성 향상 등]"
  }}
]
```
"""
